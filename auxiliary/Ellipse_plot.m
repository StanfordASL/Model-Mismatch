...
if strcmp(Type, '2D'),
    ...
    %----------------------------------------
    X_temp = V * state;
    X(1,:) = cos(th)*X_temp(1,:)'-sin(th)*X_temp(2,:)' + C(1);
    X(2,:) = sin(th)*X_temp(1,:)'+cos(th)*X_temp(2,:)' + C(2);
...
elseif strcmp(Type,'3D'),
    ...
    %------------------------------------------------------------
    XX_temp = zeros(N+1,N+1);
    YY_temp = zeros(N+1,N+1);
    ZZ_temp = zeros(N+1,N+1);
    for k = 1:length(X),
        for j = 1:length(X),
...
%     plot(C(1),C(2),'b*');
    ...
    XX = cos(th)*XX_temp-sin(th)*YY_temp + C(1);
    YY = sin(th)*XX_temp+cos(th)*YY_temp + C(2);
    ZZ = ZZ_temp + C(3);
